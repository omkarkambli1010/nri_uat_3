...
export interface CodeDisplayMaster {
  code: string;
  displayName: string;
}
...
// Body accepted by POST …/applications/{id}/passport.
...
// RM Master row (GET …/rm-master/{code}). One code can resolve to several
// relationship managers, e.g. 13623 -> 13623 / OS13623 / FTC13623.
export interface RmMasterResponse {
  workforceId: string;
...
export interface DigilockerConfirmationResponse {
  stagecode?: string;
  uiMetadata?: string;
  status?: boolean;
}
...
class APIService {
  ...
  private getHeaders(withAuth = true): Record<string, string> {
    ...
    const token =
      typeof window !== "undefined"
        ? secureSessionService.getItem("AccT")
        : null;
  ...
  private encryptPayload(data: unknown): { request: string } {
    const clientId = this.getClientId();

    return {
      request: aesService.encrypt(JSON.stringify(data), clientId, clientId),
    };
  }

  private decryptResponse<T = any>(response: AxiosResponse): T {
    const clientId = this.getClientId();
    ...
    try {
      const decryptedData = aesService.decrypt(
        encryptedData,
        clientId,
        clientId,
      );
      ...
      let parsedValue: unknown = decryptedData;
  ...
  private handleError(
    ...
    // const errorData = this.decryptResponse(error?.response?.data);

    const errorData = error?.response
      ? this.decryptResponse(error.response)
      : error?.response?.data;
    ...
    if (status === 400) {
      ...
      // if (isUnauthorized) {
      const alreadyShown = secureSessionService.getItem("Unauthorized");
      if (!alreadyShown) {
        ...
        secureSessionService.setItem("Unauthorized", "Error Message Shown");
        setTimeout(() => {
  ...
  async getMaster<T = unknown>(masterName: string): Promise<T[]> {
    ...
    try {
      ...
      let decryptedResponse: unknown = this.decryptResponse(response);
      ...
      const masterResponse = decryptedResponse as MasterResponse<T>;
  ...
  async getCountryMaster(): Promise<CountryMaster[]> {
    return this.getMaster<CountryMaster>("COUNTRYMASTER");
  }
  async getNomineeRelationshipMaster(): Promise<CodeDisplayMaster[]> {
    return this.getMaster<CodeDisplayMaster>("NOMINEERELATIONSHIP");
  }
  async getGuardianRelationshipMaster(): Promise<CodeDisplayMaster[]> {
    return this.getMaster<CodeDisplayMaster>("GUARDIANRELATIONSHIP");
  }
  async getNomineeProofTypeMaster(): Promise<CodeDisplayMaster[]> {
    return this.getMaster<CodeDisplayMaster>("NOMINEEPROOFTYPE");
  }
  async getGuardianProofTypeMaster(): Promise<CodeDisplayMaster[]> {
    return this.getMaster<CodeDisplayMaster>("GUARDIANPROOFTYPE");
  }
  ...
  async searchBankMasterByIfsc(
    ...
    try {
      ...
      const body = this.decryptResponse<any>(response);
  ...
  async getRmMasterByCode(
    code: string,
    extraHeaders?: Record<string, string>,
  ): Promise<RmMasterResponse[]> {
    const trimmed = code.trim();
    if (!trimmed) return [];

    const url = `${this.nriapi}rm-master/${encodeURIComponent(trimmed)}`;
    ...
    try {
      ...
      if (Array.isArray(body)) return body;
      if (Array.isArray(body?.data)) return body.data;
      if (body?.workforceId) return [body];
      return [];
    } catch (error) {
  ...
  async submitFatca(
    ...
    try {
      const response = await axios.post(url, this.encryptPayload(data), {
        headers: {
          ...this.getHeaders(),
          accept: "*/*",
          "Content-Type": "application/json",
  ...
  async confirmDigilocker(
    applicationId: string,
    confirmationFlag: "yes" | "no",
    hideSpinner?: () => void,
  ): Promise<DigilockerConfirmationResponse> {
    return this.postNri(
      `applications/${applicationId}/digilocker/confirmation`,
      {
        confirmationFlag,
        idempotencyKey: "",
      },
  ...
  async updatePassport(
    applicationId: string,
    data: PassportUpdatePayload | Record<string, unknown>,
    hideSpinner?: () => void,
  ): Promise<any> {
    const url = `${this.nriapi}applications/${applicationId}/passport`;
    try {
  ...
  async initiateDigilocker(
    ...
    try {
      const response = await axios.post(url, this.encryptPayload({}), {
        headers: {
          ...this.getHeaders(),
          accept: "*/*",
        },
  ...
  async getPlanSelectionWorkflow(
    ...
    return this.getWorkflowStageData(
      applicationId,
      "PLANSELECTION",
      hideSpinner,
    );
...
//     try {
//       const decryptedData = aesService.decrypt(rawData, clientId, clientId);

//       if (!decryptedData) {
//         return rawData as T;
//       }

//       let parsedValue: unknown = JSON.parse(decryptedData);
...
//   //     url,
//   //     this.encryptPayload({ masterName, idempotencyKey: "" }),
...
//     try {
//       const response = await axios.post(url, payload, { headers });
//       return await this.decryptResponse(response); //response.data;
//     } catch (error) {
//       return this.handleError(error, hideSpinner, errorOptions);
//     }
//   }
